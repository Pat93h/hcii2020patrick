...
simruns <- list.files(file.path("simulation", "dataexchange"))[2]

oldnewtest <- newtest_sim_df
...
for (runname in simruns){
  ...
  for (i in 1:max(df$TickNr)){
    ...
    if (i %% ceiling(max(df$TickNr) / 10) == 0 ) cat(paste0(round(i / max(df$TickNr) * 100), '% completed'))
    df %>%
    ...
    if (i %% ceiling(max(df$TickNr) / 10) == 9 & i == max(df$TickNr)) cat(': Done')
    else cat('\014')
  }
